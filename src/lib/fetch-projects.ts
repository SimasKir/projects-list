...
const toQuery = (filters?: FiltersType, sort?: SortType[]) => {
  ...
  if (filters?.initial_rating?.length) {
    appendJsonArrayParam(p, "filters[]", [
      { id: "initial_rating", value: filters.initial_rating },
    ]);
  }

  if (filters?.purpose?.length) {
    appendJsonArrayParam(p, "filters[]", [
      { id: "purpose", value: filters.purpose },
    ]);
  }

  const range: { min?: number; max?: number } = {};
  if (filters?.credit_duration_min != null)
    range.min = filters.credit_duration_min;
  if (filters?.credit_duration_max != null)
    range.max = filters.credit_duration_max;

  if (Object.keys(range).length > 0) {
    appendJsonArrayParam(p, "filters[]", [
      { id: "credit_duration", value: range },
    ]);
  }

  if (filters?.pid) {
    appendJsonArrayParam(p, "filters[]", [
      { id: "private_id", value: filters.pid },
    ]);
  }
